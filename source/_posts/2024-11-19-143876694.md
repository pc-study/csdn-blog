...
date: 2024-11-19 11:00:22
categories:
  - 数据库
tags:
  - 数据库
abbrlink: 143876694
csdn_url: https://luciferliu.blog.csdn.net/article/details/143876694
...
* [安装前配置](#_26)
* + [检查操作系统版本（所有节点）](#_27)
  + [配置 /etc/hosts（所有节点）](#_etchosts_40)
  + [关闭防火墙（所有节点）](#_50)
  + [禁用 selinux（所有节点）](#_selinux_60)
  + [创建主机用户（所有节点）](#_73)
  + [创建目录（所有节点）](#_90)
  + [系统参数配置（所有节点）](#_101)
  + [资源配置（所有节点）](#_163)
  + [配置 RemoveIPC（所有节点）](#_RemoveIPC_177)
  + [检查 /tmp 目录（所有节点）](#_tmp__189)
  + [配置时间同步（所有节点）](#_207)
  + [配置软件源（所有节点）](#_214)
  + [挂载共享盘（所有节点）](#_248)
  + [UDEV 绑盘（所有节点）](#UDEV__303)
* [部署 KingbaseES RAC 集群](#_KingbaseES_RAC__339)
* + [安装数据库软件（所有节点）](#_340)
  + [创建集群部署目录（所有节点）](#_489)
  + [配置 cluster\_manager.conf（所有节点）](#_cluster_managerconf_521)
  + [磁盘初始化（节点一）](#_600)
  + [基础组件初始化（所有节点）](#_650)
  + [gfs2 相关资源初始化（节点一）](#gfs2__763)
  + [初始化数据库集簇（节点一）](#_819)
  + [托管数据库（节点一）](#_861)
  + [启停集群](#_967)
  + [访问数据库](#_1083)
...
### 配置 cluster\_manager.conf（所有节点）

进入 KingbaseHA 目录，修改集群搭建必要参数：

![](https://img-blog.csdnimg.cn/img_convert/2a6e63f3f6de43db3c9020d0a767379a.png)

查看 cluster\_manager.conf 默认配置：
...
启动停止集群需要在 root 用户下执行，使用 cluster\_manager.sh 脚本。
