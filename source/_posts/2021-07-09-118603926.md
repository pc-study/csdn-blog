...
tags:
  ...
  - gap
abbrlink: 118603926
csdn_url: https://luciferliu.blog.csdn.net/article/details/118603926
description: '前言 DG GAP 顾名思义就是：DG不同步，当备库不能接受到一个或多个主库的归档日志文件时候，就发生了 GAP。 那么，如果遇到GAP如何修复呢？且听我细细道来~ 一、介绍 DG GAP 主要分为以下两类情况： 1、主库归档日志存在，可以通过配置 Fetch Archive Log(FAL) 参数'
keywords: '数据库, dataguard, gap'
...
> RECOVER STANDBY DATABASE FROM SERVICE primary\_db;
