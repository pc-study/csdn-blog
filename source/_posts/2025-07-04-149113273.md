...
tags:
  ...
  - 数据库
abbrlink: 149113273
csdn_url: https://luciferliu.blog.csdn.net/article/details/149113273
description: '大家好，这里是 DBA 学习之路，专注于提升数据库运维效率。 目录 前言 脚本演示 测试环境准备 部署并执行自动扩展脚本 配置定时任务 (crontab) 写在最后 前言 最近有客户反馈了一个困扰已久的问题：频繁收到表空间使用率超过 90% 的告警邮件，每次都需要手动添加数据文件来解决。客户询问：O'
keywords: 'oracle, 数据库'
cover: '/images/3e0eaa46e3f81a3bf5a28bfbbb804ebe.webp'
---
...
![表空间使用率接近100%](/images/3e0eaa46e3f81a3bf5a28bfbbb804ebe.webp)

### 部署并执行自动扩展脚本

1、**上传脚本**：将 `tbs_autoext.sh` 脚本上传到服务器指定目录（例如 `/home/oracle/tbs_check/`）

![脚本上传示意](/images/a7044d4fcafa50b96b9ad6cd7a9d02d7.webp)
...
![脚本执行输出示意](/images/7c5d313ab291cd3af0f959a0ca256176.webp)

3、**查看执行日志**：脚本运行完成后，可通过日志文件查看详细的操作记录

![日志文件内容示意](/images/2e5dbbf4c39e6b28c505525b27f67255.webp)
