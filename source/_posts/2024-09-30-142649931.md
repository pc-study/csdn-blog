...
tags:
  ...
  - 达梦
abbrlink: 142649931
csdn_url: https://luciferliu.blog.csdn.net/article/details/142649931
description: '大家好，这里是公众号 DBA学习之路，分享一些学习国产数据库路上的知识和经验。 前言 达梦是国产数据库中第一家上市的公司，在未来信创路上应该会越走越顺，所以对于国产数据库学习比较迷茫的同学，可以考虑先学习达梦数据库。 学习任何一种数据库之前，都需要先安装好数据库环境，所以安装是 DBA 的必备技能之'
keywords: '数据库, ffmpeg, 达梦'
...
| --- | --- | --- | --- | --- | --- |
| dm8 | 192.168.6.145 | rhel7.9 | dm8\_20240712 | 100G | 8G |

关于 Linux 主机安装，建议不要使用最小化安装，达梦安装对于一些命令是有要求的，最小化不包含这些命令，比如 `tar` 命令。

## 安装包下载

当前达梦官方提供的最新版本为 DM8\_20240712，可以在 **[达梦官网](https://eco.dameng.com)** 直接下载：

> **达梦8的数据库安装介质下载地址：**[https://eco.dameng.com/download](https://eco.dameng.com/download/)

![](https://img-blog.csdnimg.cn/img_convert/d7ba64f485f88ae430a9262553c552e8.png)
...
limits.conf 文件实际是 Linux PAM （插入式认证模块，Pluggable Authentication Modules 中 pam\_limits.so 的配置文件），突破系统的默认限制，对系统访问资源有一定保护作用，建议配置如下：
...
将官网下载的安装包上传到安装主机上，并进行解压，会得到一个 dm8\_20240712\_x86\_rh7\_64.iso 镜像文件，也就是达梦安装镜像，需要挂载后进行安装：
...
![](https://img-blog.csdnimg.cn/img_convert/1b242de47a9e29c53d41332c8d0a189f.png)

2、按需求选择安装语言，默认为中文。本地安装选择【不输入 Key 文件】，选择【默认时区 21】。

![](https://img-blog.csdnimg.cn/img_convert/11935860b625c88daaee1f1bd7485b91.png)

3、选择【1-典型安装】，按已规划的安装目录 /dm 完成数据库软件安装，不建议使用默认安装目录。

![](https://img-blog.csdnimg.cn/img_convert/18087b86c5ddcd88aec3b1b3186ffcac.png)
