...
tags:
  ...
  - 云计算
abbrlink: 154742377
csdn_url: https://luciferliu.blog.csdn.net/article/details/154742377
description: '目录 前言 阿里云 OSS 日志备份 快照备份 写在最后 前言 最近好久没更新 Oracle 相关内容了，都在搞备份的东西，为啥呢？ 因为最近 Oracle 相关的内容都是很基础的，也没有遇到什么问题，所以就自然没什么内容可写了，反倒是最近几天都在研究 TiDB 备份，顺便学习了下 TiDB 的体系'
keywords: 'tidb, 阿里云, 云计算'
cover: '/images/6977003e7e707502fed50cd1114a2f03.webp'
---
#### 目录
...
![使用 BR 备份 TiDB 到阿里云 OSS 存储](/images/6977003e7e707502fed50cd1114a2f03.webp)

如果使用 `S3 Browser` 记得要选择 `Virtual hosted style`：

![使用 BR 备份 TiDB 到阿里云 OSS 存储 - 图2](/images/efb36a4c82955cdd7db9be7dc7cfe969.webp)

阿里云 OSS 不支持 `Path style`，会报错：

![使用 BR 备份 TiDB 到阿里云 OSS 存储 - 图3](/images/a2c1ed4311607373d01d3321a1e70ba5.webp)

连接后可以看到创建好的 OSS 存储以及里面的内容，测试可以正常上传文件：

![使用 BR 备份 TiDB 到阿里云 OSS 存储 - 图4](/images/367abbdea19194dabc2b766d4ebd3a8f.webp)
...
![使用 BR 备份 TiDB 到阿里云 OSS 存储 - 图5](/images/ec78baff1e3d79fe8d1d98229c40b7d1.webp)

是因为在 provider 是 alibaba 的情况下，没有使用到环境变量中 AK/SK 的问题导致的这个报错，但是在 **BR v7.5.3** 版本已经进行修复：

![使用 BR 备份 TiDB 到阿里云 OSS 存储 - 图6](/images/98cc07a52cdb6916e8e51fbf6a83251a.webp)
...
![使用 BR 备份 TiDB 到阿里云 OSS 存储 - 图7](/images/d79668c15893073180573777965306e5.webp)
