...
tags:
  ...
  - gbase 8a
abbrlink: 143621549
csdn_url: https://luciferliu.blog.csdn.net/article/details/143621549
...
* [社群交流](#_3)
* [前言](#_14)
* [GBase 8a 介绍](#GBase_8a__19)
* [环境准备](#_55)
* + [主机环境](#_56)
  + [安装介质下载](#_79)
* [安装前准备](#_86)
* + [关闭防火墙](#_88)
  + [禁用 Selinux](#_Selinux_96)
  + [创建 DBA 用户](#_DBA__105)
  + [创建安装目录](#_111)
  + [上传安装介质](#_119)
  + [参数一键配置](#_135)
  + [安装必备依赖](#_257)
  + [配置最大进程数](#_311)
  + [配置透明大页和 I/O 调度参数调整](#_IO__327)
* [GBase 8a MPP Cluster 安装](#GBase_8a_MPP_Cluster__367)
* + [安装集群软件](#_373)
  + - [配置 demo.options](#_demooptions_375)
    - [执行 gcinstall.py](#_gcinstallpy_422)
    - [检查集群状态](#_481)
  + [创建数据分布模式](#_529)
  + - [修改 gcChangeInfo.xml](#_gcChangeInfoxml_542)
    - [创建 distribution](#_distribution_557)
  + [初始化集群](#_629)
  + [创建库表](#_654)
  + [安装后检查](#_700)
* [集群卸载](#_711)
...
![](https://img-blog.csdnimg.cn/img_convert/6ee06cbaf1059004516c6944955ab83e.png)
...
![](https://img-blog.csdnimg.cn/img_convert/70c020627b04611f60b71d1f5f01f05c.png)
...
* **GCware\_Monit**：用于实时监测 GCware 组件的运行状态, 一旦发现服务进程状态发生变化，就根据配置文件中的内容来执行相应的服务启动命令，从而保证服务组件正常运行。
* **GCRecover & GCSyncServer**：用于多副本间的数据同步。一旦发生多副本间数据文件不一致则调用该进程进行同步，从而保证多副本数据文件的一致性。

**GBase 8a MPP Cluster 产品概念图：**

![](https://img-blog.csdnimg.cn/img_convert/c857ac61f20f8cf7595c7fa11a49e5e9.png)
...
* **逻辑概念划分**：
  ...
  + **GCware Cluster**：集群的分布式管理集群，是集群的一致性管理节点集合。GCware Cluster 的节点上运行 gcware、gcware\_monit、gcware\_mmonit 服务。
  + **Data Cluster**：集群的分布式数据存储计算集群，是集群的数据存储计算节点集合。Data Cluster 的节点上运行有 gbased、gc\_sync\_server、gcmonit、gcmmonit 服务。
* **虚拟概念划分**：
...
![](https://img-blog.csdnimg.cn/img_convert/8059d03ec38d47961713f12af3c811cc.png)
...
![](https://img-blog.csdnimg.cn/img_convert/a8da37e59bc2fd0cb5f1157d9d59efc2.png)
...
![](https://img-blog.csdnimg.cn/img_convert/818ddb459484d9058a2c5887c3de8410.png)
...
官网可以直接下载：[GBase8a\_MPP\_Cluster-NoLicense-FREE-9.5.3.28.12-redhat7-x86\_64.tar.bz2](https://www.gbase.cn/download/gbase-8a?category=INSTALL_PACKAGE)。

根据操作系统的版本（同一套集群使用相同版本操作系统）获取相应的 GBase 8aMPP Cluster 软件安装包：

![](https://img-blog.csdnimg.cn/img_convert/f422eff738dbf48788cb59650580f4c6.png)
...
![](https://img-blog.csdnimg.cn/img_convert/d5cdf2cadf171682244fbab36fdf4138.png)
...
所有节点均需执行，修改 /etc/default/grub 文件，在 GRUB\_CMDLINE\_LINUX 中添加或修改参数 `transparent_hugepage=never` 和 `elevator=deadline`：
...
* UEFI：`/boot/efi/EFI/\<distro_name>/grub.cfg`，distro\_name 为系统发行版本名称，例如 ubuntu、fedora、debian 等。
...
![](https://img-blog.csdnimg.cn/img_convert/c747db4a735f078356612fbff1db0076.png)
...
![](https://img-blog.csdnimg.cn/img_convert/20540d070e6cf23e2d0d94aa1ddfaf8f.png)
...
安装完成后，在家目录下会生成一个 .gbase\_profile 环境变量文件：
...
![](https://img-blog.csdnimg.cn/img_convert/d555244f275d94f7988a8e23e227cec7.png)
