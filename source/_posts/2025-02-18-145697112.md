...
date: 2025-02-18 09:11:40
categories:
  - 多媒体处理
tags:
  - ffmpeg
abbrlink: 145697112
csdn_url: https://luciferliu.blog.csdn.net/article/details/145697112
description: '大家好，这里是公众号 DBA学习之路，分享一些学习国产数据库路上的知识和经验。 目录 前言 KingbaseRWC 介绍 环境准备 安装前配置 检查操作系统版本（所有节点） 配置 /etc/hosts（所有节点） 关闭防火墙（所有节点） 禁用 selinux（所有节点） 创建主机用户（所有节点） 创'
keywords: 'ffmpeg'
cover: '/images/f6a10dd381de1c9f2531f5b4233a3025.webp'
---
...
* [前言](#_4)
* [KingbaseRWC 介绍](#KingbaseRWC__8)
* [环境准备](#_29)
* [安装前配置](#_37)
* + [检查操作系统版本（所有节点）](#_38)
  + [配置 /etc/hosts（所有节点）](#_etchosts_51)
  + [关闭防火墙（所有节点）](#_61)
  + [禁用 selinux（所有节点）](#_selinux_71)
  + [创建主机用户（所有节点）](#_84)
  + [创建目录（所有节点）](#_100)
  + [系统参数配置（所有节点）](#_111)
  + [资源配置（所有节点）](#_168)
  + [配置 RemoveIPC（所有节点）](#_RemoveIPC_184)
  + [检查 /tmp 目录（所有节点）](#_tmp__201)
  + - [配置环境变量（所有节点）](#_219)
  + [配置时间同步（所有节点）](#_242)
* [部署 KingbaseRWC 集群](#_KingbaseRWC__247)
* + [安装数据库软件（主节点）](#_248)
  + [部署集群（主节点）](#_301)
  + - [配置 install.conf](#_installconf_302)
    - [SSH 免密配置](#SSH__315)
    - [集群静默安装](#_347)
  + [检查集群](#_619)
  + [启停集群](#_635)
...
![金仓 KES RWC（读写分离集群）静默安装部署指南](/images/f6a10dd381de1c9f2531f5b4233a3025.webp)
...
![金仓 KES RWC（读写分离集群）静默安装部署指南 - 图2](/images/f0bb484ab7b689fd3a2b167cadb7f83d.webp)
...
| rmem\_default | 8388608 | /proc/sys/net/core/rmem\_default |
| rmem\_max | 16777216 | /proc/sys/net/core/rmem\_max |
| wmem\_default | 8388608 | /proc/sys/net/core/wmem\_default |
| wmem\_max | 16777216 | /proc/sys/net/core/wmem\_max |
...
export LD_LIBRARY_PATH=$KINGBASE_HOME/lib:/lib:/usr/lib
...
静默安装需要配置免密，使用 root 用户执行 trust\_cluster.sh 脚本即可：
...
这里指定的 `-l` 值是 `${data_directory}/../logfile`，这个 data\_directory 是获取的 repmgr.conf 配置文件值：
...
那这看起来不就是 data\_directory 的值取错了吗？应该是取 KINGBASE\_HOME 环境变量才对吧！
...
好家伙，修改一下 sys\_monitor.sh 脚本，将 sys\_ctl 启停数据库命令中的 `${data_directory}/../logfile` 替换为 `${KINGBASE_HOME}/logfile` 即可：
...
![金仓 KES RWC（读写分离集群）静默安装部署指南 - 图3](/images/259ee64e565d17c50673e7a036464f53.webp)

![金仓 KES RWC（读写分离集群）静默安装部署指南 - 图4](/images/7a4194acb3258645f450bd82ff862bd0.webp)
