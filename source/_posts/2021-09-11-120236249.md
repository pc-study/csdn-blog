...
tags:
  ...
  - 数据库
abbrlink: 120236249
csdn_url: https://luciferliu.blog.csdn.net/article/details/120236249
description: '以下总结了关于 Oracle 数据库临时表空间的相关 SQL 语句： Oracle 临时表空间创建和添加数据文件： 查看当前默认临时表空间： 查询temp表空间使用率： 查看临时表空间对应的临时文件的使用情况： 查询实时使用temp表空间的sql\id和sid： 查询历史的temp表空间的使用的SQ'
