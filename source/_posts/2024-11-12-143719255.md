...
tags:
  ...
  - 达梦
abbrlink: 143719255
csdn_url: https://luciferliu.blog.csdn.net/article/details/143719255
...
* [介绍](#_18)
* [迁移准备](#_38)
* + [Oracle 19C 一键安装](#Oracle_19C__46)
  + [DM8 一键安装](#DM8__88)
  + [连接源数据库](#_152)
  + [连接目标数据库](#_161)
  + [打开迁移功能](#_172)
...
* **迁移评估**：迁移评估环节通过分析数据库对象、大表、大字段表等迁移重难点情况，采集源库对象的 SQL 语句，评估语法兼容性，生成准确的改造工作量和迁移策略。  
   ![](https://img-blog.csdnimg.cn/img_convert/fa6e9f32dbc702a1ac6c3f5c1c7d4639.png)
* **数据迁移**：数据迁移环节提供一站式全自动迁移，以合理的策略完成对象结构迁移、全量数据迁移，并对迁移效果进行一致性验证，采取任务式管理方式保障迁移工作完成。  
   ![](https://img-blog.csdnimg.cn/img_convert/c5e237e5096069f147fa523a3e4c0762.png)
...
Oracle 使用一键安装脚本部署：**<https://www.modb.pro/course/148>**
...
达梦一键安装脚本下载地址：**<https://gitee.com/hnyuanzj/DMShellInstall>**
...
* **-es [EXTENT\_SIZE]**：簇大小默认为 16 页，建议设置成 32 页。
* **-ps [PAGE\_SIZE]**：页大小默认为 8K，建议设置成 32K，一条记录的长度，受到页大小的限制，不可以超过页大小的一半，所以建议一开始规划页大小为 32K。
* **-cs [CASE\_SENSITIVE]**：默认是大小写敏感，源端为 Oracle 情况下，建议保持默认大小写敏感即可。
* **-c [CHARSET]**：字符集编码，可选 GB18030、UTF-8，默认为 GB18030，如果只存储中文和字母数字，使用 GB18030 更节省空间。
* **-bpm [BLANK\_PAD\_MODE]**：空格填充参数，是否要兼容 Oracle 进行设置，即在 BLANK\_PAD\_MODE = 0 的情况下，’A’ 和 ’A ’ 被认为是相同的值，参数为 1 的情况下，认为是两个不同的值，根据现场具体应用的需求进行设置。此为初始化参数，只能在初始化时候指定，后续不可以修改，需要提前做好评估，但是源端为 Oracle 数据库的时候，建议设置为 1。
* **-cm [COMPATIBLE\_MODE]**：兼容参数，置为 2 为兼容 Oracle 参数，在 dm.ini 中可以进行配置，如果默认 0 的情况下，达梦视 null 不等同于空字符串，用户可能会插入空串，会导致 is null 查不出全部数据，所以视情况而定是否需要修改。

**在 SQLark 正式迁移之前也会验证这些参数是否符合要求：**

![](https://img-blog.csdnimg.cn/img_convert/4fb208474d161e71b6a714a333f9d200.png)
...
![](https://img-blog.csdnimg.cn/img_convert/d8def515c68f521b2ff122bf070d7cda.png)

保存连接，双击即可连接 Oracle 19C 数据库：

![](https://img-blog.csdnimg.cn/img_convert/905ab4ec73d088addb1df7f2cbca22a5.png)

### 连接目标数据库

数据源选择 **达梦**，填写关键信息，测试连接：

![](https://img-blog.csdnimg.cn/img_convert/a7579cfacc205c5f334fe179754100e3.png)

保存连接，双击即可连接 DM8 数据库：

![](https://img-blog.csdnimg.cn/img_convert/fe99b3cd20ebc57d426c7584d13fbc1f.png)
...
![](https://img-blog.csdnimg.cn/img_convert/2dcf4de4d04d923cd7c4fbd10a0e86ce.png)

打开后会跳转到网页端：

![](https://img-blog.csdnimg.cn/img_convert/7ac8350ffa43281561a9ed78bafe3b03.png)
...
![](https://img-blog.csdnimg.cn/img_convert/562baeeb2a2312090242868b8e97262e.png)
...
![](https://img-blog.csdnimg.cn/img_convert/f158433406fecd25fe04f170ef567c1e.png)
...
![](https://img-blog.csdnimg.cn/img_convert/17cecb69d376eb8a39e3f92457e3886a.png)
...
![](https://img-blog.csdnimg.cn/img_convert/295b9a888be6d6095a4f2d45ead4f826.png)
...
![](https://img-blog.csdnimg.cn/img_convert/11da470b5f38424ab10dda628d49ea5e.png)
...
![](https://img-blog.csdnimg.cn/img_convert/c8f43fd5e94d6d2e3b523c3fc2d75633.png)

点击页面右上方 **导出策略**，即可将迁移策略报告导出为 **PDF 格式**，为正式迁移实施提供重要参考。

在待迁移模式统计模块，点击 查看迁移步骤， 查看该模式下数据库对象和表数据的自动化迁移顺序：

![](https://img-blog.csdnimg.cn/img_convert/9f126362af7b2d4c614fa3445f28a97b.png)

![](https://img-blog.csdnimg.cn/img_convert/9c3bf74ddccbcc2aa415b0146f4dc988.png)

![](https://img-blog.csdnimg.cn/img_convert/6acf28e9d8de1c6bea09e39acb5f9810.png)
...
![](https://img-blog.csdnimg.cn/img_convert/6f3f90bcd8bb9b7ddd84cad12181f485.png)

### 连接目标数据库

确认连接信息无误后，点击 **测试连接**，显示 **数据库测试连接成功** 信息后即可成功连接。

![](https://img-blog.csdnimg.cn/img_convert/c06d000a056f63546968b1f944a3fece.png)

测试连接成功后，点击 **下一步，确认迁移范围/配置**，即可连接至目标库：

![](https://img-blog.csdnimg.cn/img_convert/45808a838d0d014c36843bbe0070faa2.png)
...
![](https://img-blog.csdnimg.cn/img_convert/8139728770f86c1ef3893ac81163131e.png)

为提升迁移效率，SQLark 根据当前部署环境自动生成以下最佳迁移参数，参数确认无误后，点击 **确认** 按钮：

![](https://img-blog.csdnimg.cn/img_convert/3c1de136d379301009199ea04578f7d9.png)
...
![](https://img-blog.csdnimg.cn/img_convert/9c7d3ccb79f54994651ba5bed3e12a64.png)
...
![](https://img-blog.csdnimg.cn/img_convert/86c84d76e63516f9535b0a13dc1746f4.png)

整个迁移任务中，未迁移成功的对象和表数据，均集中在待处理清单中解决。点击待处理清单模块的 **去处理**，主要包括以下三类对象：不兼容对象、异常对象和异常表数据。

![](https://img-blog.csdnimg.cn/img_convert/48016184db04eb92eab1ff3676c69dd7.png)

点击右下角的【改写对象】，在弹出的 SQL 编辑器中，改写语句执行成功后，请务必点击【验证】，SQLark 将验证该对象是否在目标库成功创建：

![](https://img-blog.csdnimg.cn/img_convert/c2eca0e6f1ec0acf09cd283266a41d6a.png)

所有待处理清单处理完毕后 ，可点击 **下一步，开启迁移校验**：

![](https://img-blog.csdnimg.cn/img_convert/ef40a7fbcecac53ca74ac852b8ab11a8.png)

若在迁移过程中发生错误或意外情况，点击页面左下方 **迁移日志**，可查看迁移过程中的全部日志和异常日志：

![](https://img-blog.csdnimg.cn/img_convert/11988dfb63f01b98485ad59aa9b54b27.png)
...
![](https://img-blog.csdnimg.cn/img_convert/8fe640190c1c2e9f9bf6951a95a82ce4.png)

右上角的 **生成完整迁移报告** 功能暂未开放。

达梦数据库中随便找一张迁移后的表看看数据：

![](https://img-blog.csdnimg.cn/img_convert/866208f7f849cd7e50a04aa265304d92.png)
