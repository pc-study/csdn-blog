...
tags:
  ...
  - GBase 8s
abbrlink: 143171024
csdn_url: https://luciferliu.blog.csdn.net/article/details/143171024
description: '大家好，这里是公众号 DBA学习之路，分享一些学习国产数据库路上的知识和经验。 目录 前言 单选题 多选题 判断题 写在最后 前言 近期，南大通用 GBase 8s 数据库新一期的 GDCA 认证开启火热报名：培训预告 2024十月GBase 8s认证培训班开始报名啦~，到今天为止，培训课程已经学习'
keywords: '数据库, GBase 8s'
...
* C dbaccess 默认不支持显示 SQL 的执行时间，但可通过设置环境变量 GL\_SHOWTIME 来支持。
* D dbaccess 默认不支持显示 SQL 的执行时间，但可通过设置环境变量 DBACCESS\_SHOW\_TIME 来支持。
...
* C unload 可以导出查询中的数据到外部文本文件。当数据中包含换行符时，会将数据导出成多行，在因数据内换行符而产生换行的行尾,自动加上一个 `\`，表示这一行没有结束。
...
* D 在 mydb 数据库中执行 `select * from systables where tabid>= 100 and tabtype='V';`
...
**解析:** WM\_CONCAT 是特殊的聚合函数，可以实现某列的字串拼接。
...
* B 在mydb数据库中执行 `select colname from syscolumns where tabid in(select tabid from systables where tabname='mytab');`
* C 在systemaster数据库中执行 `select colname from syscolumns where dbname='mydb' and tabname='mytab';`
* D 在mydb数据库中执行 `info columns for mytab;`
...
**解析：** `onstat -c` 可以直接显示数据库的配置文件内容，也可以进入 GBase 8s 的安装目录中的 etc 目录中，直接查看 onconfig 文件中的内容。GBase 8s 的配置文件保存在一个类似 onconfig.<server\_name> 的文本文件中。
