...
tags:
  ...
  - 数据库
abbrlink: 146302237
csdn_url: https://luciferliu.blog.csdn.net/article/details/146302237
description: '前言 Oracle 数据库有 db\name 和 db\unique\name 的区别，在搭建 DG 的时候，要求主备的 db\unique\name 必须不一致，所以我们通过 DG 的方式迁移数据库后，通过切换后新的主库 db\unique\name 会与原主库不一致。 这时候，如果想修改 db\'
keywords: 'oracle, 数据库'
cover: '/img/covers/oracle.svg'
---
## 前言

Oracle 数据库有 db\_name 和 db\_unique\_name 的区别，在搭建 DG 的时候，要求主备的 db\_unique\_name 必须不一致，所以我们通过 DG 的方式迁移数据库后，通过切换后新的主库 db\_unique\_name 会与原主库不一致。

这时候，如果想修改 db\_unique\_name，该怎么做呢？

通常是切换成功后，手动修改新主库的 db\_unique\_name：
...
手工修改参数文件中的 db\_unique\_name：

```
*.db_unique_name=luciferdg
```

如果存在 db\_domain 配置，也需要修改（我这里没有配置，不做修改）：
...
* [How to change the DB\_UNIQUE\_NAME in a RAC database (Doc ID 1604421.1)](https://support.oracle.com/epmos/faces/DocContentDisplay?id=1604421.1)