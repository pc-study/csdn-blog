...
tags:
  ...
  - database
abbrlink: 122819219
csdn_url: https://luciferliu.blog.csdn.net/article/details/122819219
description: '原文地址： 原文作者：Tim Hall 在 Oracle 数据库 21c 中配置 Data Guard 时，使用 命令可以简化主数据库的设置。 目录 介绍 PREPARE DATABASE FOR DATA GUARD 介绍 在配置 Data Guard 之前，主数据库的准备需要执行许多步骤。对于一'
...
* [介绍](#_7)
* [PREPARE DATABASE FOR DATA GUARD](#PREPARE_DATABASE_FOR_DATA_GUARD_42)

## 介绍

在配置 Data Guard 之前，主数据库的准备需要执行许多步骤。对于一个简单的单实例主数据库，可能需要以下步骤：

设置 DB\_RECOVERY\_FILE\_DEST\_SIZE 和 DB\_RECOVERY\_FILE\_DEST 初始化参数：
