...
tags:
  ...
  - SSC
abbrlink: 144443754
csdn_url: https://luciferliu.blog.csdn.net/article/details/144443754
description: '大家好，这里是公众号 DBA学习之路，致力于分享数据库领域相关知识。 社群交流 为了给大家提供一些技术交流的平台，目前已成立的技术交流群： Oracle 数据库交流群 国产数据库交流群 Linux 技术交流群 ChatGPT 4o 免费体验群 需要进群可以添加号主微信：Lucifer-0622，备注'
keywords: '数据库, GBase, SSC'
cover: '/images/3d0129674f40fb31483af3dc7f8790b5.webp'
---
...
* + [主机配置规划](#_60)
  ...
  + [安装包下载](#_77)
* [安装前准备](#_99)
* + [配置 hosts 文件](#_hosts__102)
  + [优化 DNS 配置](#_DNS__135)
  + [配置防火墙](#_156)
  + [关闭 SELinux](#_SELinux_182)
  + [关闭 RemoveIPC](#_RemoveIPC_198)
  + [创建组和用户](#_213)
  + [创建安装目录及授权](#_237)
  + [资源配置](#_246)
  + [安装依赖包](#_267)
  ...
  + [创建裸设备链接](#_613)
* [SSC 集群安装](#SSC__645)
* + [安装 GBase 8s 软件](#_GBase_8s__646)
  + [安装 CSDK](#_CSDK_669)
  + [环境变量配置](#_693)
  + [配置互信](#_716)
  + - [配置 .rhosts 文件](#_rhosts__723)
    - [配置 REMOTE\_SERVER\_CFG 参数](#_REMOTE_SERVER_CFG__740)
  + [配置 sqlhosts 文件](#_sqlhosts__764)
  + [配置 onconfig 文件](#_onconfig__788)
  + [搭建集群关系](#_875)
  + [创建默认数据库](#_965)
  + [配置集群主节点](#_974)
  + [启动辅节点库](#_989)
...
* + [CM 介绍](#CM__1260)
  ...
  + - [重定向方式](#_1272)
    - [代理方式](#_1277)
  + [配置 CM 文件](#_CM__1282)
  + [修改 ONCONFIG 文件](#_ONCONFIG__1327)
  + [启动 CM](#_CM_1344)
* [写在最后](#_1393)
* [往期精彩文章](#_1398)
...
![实战篇：一步步教你搭建 GBase 8s SSC 共享集群](/images/b7b6ff4ae2543b6a5e24c58f3356a0a7.webp)
...
![实战篇：一步步教你搭建 GBase 8s SSC 共享集群 - 图2](/images/3d0129674f40fb31483af3dc7f8790b5.webp)
...
![实战篇：一步步教你搭建 GBase 8s SSC 共享集群 - 图3](/images/11a707fe4390b0909872320bb309e6f0.webp)

GBase 8s 共享存储集群 SSC 最少 2 个节点，最多 128 节点，各节点关系对等，以下为单节点推荐配置：

![实战篇：一步步教你搭建 GBase 8s SSC 共享集群 - 图4](/images/0cad3e8476c13e2571ea71103ac2bc7f.webp)
...
> **Gbase 8s V8.8 x86 的数据库安装介质下载地址：**[GBase8sV8.8\_TL\_3.5.1\_3X2\_x86\_64.tar](https://www.gbase.cn/download/gbase-8s-1?category=INSTALL_PACKAGE)

![实战篇：一步步教你搭建 GBase 8s SSC 共享集群 - 图5](/images/56834a1372b955b482397840acaec091.webp)
...
> **libnsl-2.28-98.p02.ky10.x86\_64.rpm 下载：** <https://update.cs2c.com.cn/NS/V10/V10SP3-2403/os/adv/lic/base/x86_64/Packages/libnsl-2.28-98.p02.ky10.x86_64.rpm>
...
修改 /etc/default/grub 文件，在 GRUB\_CMDLINE\_LINUX 中添加或修改参数 transparent\_hugepage=never：
...
* UEFI：/boot/efi/EFI/<distro\_name>/grub.cfg，distro\_name 为系统发行版本名称，例如 ubuntu、fedora、debian 等。
...
7. **comnspace**：磁盘心跳空间（ssc\_alt\_comm），共享存储集群需要在共享存储上创建一个用于通信的 dbspace（5GB）。
...
![实战篇：一步步教你搭建 GBase 8s SSC 共享集群 - 图6](/images/0920ed2a25ac90cfec783b393a7f8054.webp)
...
![实战篇：一步步教你搭建 GBase 8s SSC 共享集群 - 图7](/images/30f70488ba86dd85bd61ac398d6bc622.webp)

#### 代理方式

在代理方式下，连接管理器充当代理服务器，并管理客户机与数据库服务器的通信。当客户机应用程序无法连接到位于防火墙后面的数据库服务器时，请使用代理方式。要避免连接管理器成为单个故障点，在使用代理方式时，请配置多个连接管理器实例。

![实战篇：一步步教你搭建 GBase 8s SSC 共享集群 - 图8](/images/db705b6da1e1b80ec49334207a3ce898.webp)
...
配置完 CM 文件，我们还需确认一下 ONCONFIG 文件中的两个参数：**DRAUTO**、**HA\_FOC\_ORDER**。

* **DRAUTO 参数**：确定故障切换方式的参数，部署 CM，让 CM 来控制故障转移，需要把 DRAUTO 参数设置成 3。
* **HA\_FOC\_ORDER 参数**：故障转移规则，该值默认值为 SDS/HDR/RSS；也就是说如果主节点发生故障，先转移到 SSC 辅助服务器，然后是 HAC 辅助服务器，再然后是  
   RHAC 辅助服务器。。
...
可以看到，HA\_FOC\_ORDER 参数未作修改，用默认值的时候，发生故障，故障转移的顺序为 SSC、HAC、RHAC。
